...
class EditDistance:
    ...
    def edit_distance(self, query):
        ...
        for term in query_terms.items():
            if not term[1] in self.dictionary.terms: # don't look for suggestions if word is recognized 
    ...
    def __get_suggestions(self, term):
        ...
        for word in self.dictionary.terms: 
            ...
            if word[0] == term[0] and not any(char not in self.key_locations.keys() for char in word.lower()):
    ...
    def __substitution_cost(self, key1, key2):
        if not((key1 in self.key_locations.keys()) and (key2 in self.key_locations.keys())):
